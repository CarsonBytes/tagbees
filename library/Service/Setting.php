...
class Service_Setting {
  ...
  public function get($id = '') {
    ...
    $select = $this -> db -> select() -> from(array('user')) -> joinLeft('item', 'item.id=user.item_id', array('name', 'description', 'place', 'lat', 'lng', 'zoom')) -> joinLeft('user_account', 'user_account.user_id = user.item_id', array('identifiers' => new Zend_Db_Expr('GROUP_CONCAT(identifier)'), 'provider_ids' => new Zend_Db_Expr('GROUP_CONCAT(provider_id)'))) -> joinLeft('account_provider', 'account_provider.id = user_account.provider_id', array('account_provider_names' => new Zend_Db_Expr('GROUP_CONCAT(account_provider.name)'))) -> where('item_id= ?', $id) -> group('user.item_id');
  ...
  public function checkIfUpdated($country = 'CN', $city = 'Hong Kong') {
    ...
    $select = $this -> db -> select() -> from(array('weather'), array('city')) -> where('country = ?', $country) -> where('city = ?', $city) -> where('update_time > (NOW() - INTERVAL 1 HOUR)');
  ...
  private function fetch($country, $city) {
    ...
    $values = array('country' => $country, 'city' => $city, 'data' => $data);
  ...
  private function saveToUser($locations) {
    $this -> db -> update('user', array('weather_locations' => json_encode($locations)), 'item_id=' . $this -> identity -> item_id);
  }
  ...
  public function save($data) {
    ...
    $result = $this -> db -> update('item', $item_data, 'id=' . $this -> identity -> item_id);
    $result = $this -> db -> update('user', $user_data, 'item_id=' . $this -> identity -> item_id);
  }
