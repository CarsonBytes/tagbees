	...
	public function toString($indent = null) {
		...
		foreach ( $this as $item ) {
			...
			if (isset($item->attributes ['src']) && !empty($item->attributes ['src']) && strpos($item->attributes ['src'], 'http://') === false) {
				...
				//for mobile we need to replace again here using '//js/'
				$scripts [] = str_replace(array($baseUrl,'//js/'), '', $item->attributes ['src']);
			} else {
				if (count($scripts) > 0) {
					...
					//} else {
						//just treak the url a bit...
						//$minScript->attributes ['src'] = $this->getMinUrl() . '?b=' . $baseUrl . '&f=' . implode(',', $scripts);
						$minScript->attributes ['src'] = $this->getMinUrl() . '?b=js&f=' . implode(',', $scripts);
					//}
